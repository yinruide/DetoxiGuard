...
eval_llama.py
-------------
Standalone evaluation script for the fine-tuned LLaMA+LoRA toxic comment
...
All figures are saved to  outputs/eval_llama/

Usage (inside Singularity on HPC with GPU):
    python evaluation/eval_llama.py \
        --checkpoint  outputs/llama_lora/best_checkpoint \
...
# Adjust sys.path so `classifier.train_llama` is importable when running
# from the repo root (python evaluation/eval_llama.py).
REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if REPO_ROOT not in sys.path:
    sys.path.insert(0, REPO_ROOT)

from classifier.train_llama import (
    LABELS,
    load_trained_model,
    predict,
)
...
def plot_roc_curves(labels_arr, probs_arr, save_path):
    ...
    ax.set_title("Per-label ROC Curves — LLaMA+LoRA")
...
def plot_pr_curves(labels_arr, probs_arr, save_path):
    ...
    ax.set_title("Per-label Precision-Recall Curves — LLaMA+LoRA")
...
def plot_confusion_matrices(labels_arr, probs_arr, thresholds, save_path):
    ...
    fig.suptitle("Confusion Matrices (optimal thresholds) — LLaMA+LoRA", fontsize=13)
...
def plot_prob_distributions(labels_arr, probs_arr, thresholds, save_path):
    ...
    for i, label in enumerate(LABELS):
        ...
        ax.hist(neg_probs, bins=80, alpha=0.6, label=f"Negative (n={len(neg_probs):,})",
                color="#85B7EB", density=True)
        ax.hist(pos_probs, bins=80, alpha=0.6, label=f"Positive (n={len(pos_probs):,})",
    ...
    fig.suptitle("Probability Distributions (positive vs negative) — LLaMA+LoRA", fontsize=13)
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(save_path, dpi=DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved: {save_path}")
...
def plot_threshold_sensitivity(labels_arr, probs_arr, thresholds, save_path):
    ...
    for i, label in enumerate(LABELS):
        ...
        # Mark optimal point
        t_opt = thresholds[label]
        f1_opt = f1_score(labels_arr[:, i],
                          (probs_arr[:, i] >= t_opt).astype(int),
                          zero_division=0)
        ax.plot(t_opt, f1_opt, "o", markersize=6, color="black", zorder=5)
        ax.annotate(f"{t_opt:.2f}", (t_opt, f1_opt),
                    textcoords="offset points", xytext=(5, 5), fontsize=7)

    ax.set_xlabel("Threshold")
    ax.set_ylabel("F1 Score")
    ax.set_title("F1 vs Threshold per Label — LLaMA+LoRA")
...
def main():
    ...
    print(f"Loaded thresholds: { {k: round(v, 2) for k, v in thresholds.items()} }")
    ...
    probs_arr = predict(model, tokenizer, texts.tolist(), batch_size=args.batch_size)
    print(f"Predictions shape: {probs_arr.shape}")

    # Save raw probabilities for downstream use (e.g., ensemble comparison)
    np.save(os.path.join(args.output_dir, "val_probs_llama.npy"), probs_arr)
    np.save(os.path.join(args.output_dir, "val_labels.npy"), labels_arr)
    print(f"  Saved: val_probs_llama.npy, val_labels.npy")
